...
public class BattleshipMatrix  implements Serializable {
    private final int[] array = {4, 3, 3, 2, 2, 2, 1, 1, 1, 1};
    public BattleshipMatrixCell[][] matrix;

    public List<Ship> ships;

    public BattleshipMatrix() {
        this.matrix = new BattleshipMatrixCell[10][10];
        this.ships = new ArrayList<>();

        for (int row = 0; row < 10; row++) {
            for (int column = 0; column < 10; column++) {
                this.matrix[row][column] = new BattleshipMatrixCell();
            }
        }
    }

    public List<List<BattleshipMatrixCell>> GetList() {
        List<List<BattleshipMatrixCell>> matrixList = new ArrayList<>();
        for (int row = 0; row < 10; row++) {
            ArrayList<BattleshipMatrixCell> listRow = new ArrayList<>(Arrays.asList(matrix[row]).subList(0, 10));
    ...
    private boolean CheckCell(int pointX, int pointY, int rotation, int length) {
        List<BattleshipMatrixCell> shipCells = new ArrayList<>();
        List<BattleshipMatrixCell> surroundCells = new ArrayList<>();
    ...
    public void CheckShip(int row, int column) {
        for(Ship ship:ships){
            for(BattleshipMatrixCell cell:ship.shipCells){
    ...
    private void SurroundShip(Ship ship) {
        for(BattleshipMatrixCell cell:ship.surroundCells){
