...
public class BattleshipMatrix {
    ...
    public BattleshipMatrix(List<List<HashMap<Object, Object>>> fieldList) {
        this.matrix = new BattleshipMatrixCell[10][10];

        for (int row = 0; row < 10; row++) {
            for (int column = 0; column < 10; column++) {
                this.matrix[row][column] = new BattleshipMatrixCell((boolean) fieldList.get(row).get(column).getOrDefault("isHead", false),
